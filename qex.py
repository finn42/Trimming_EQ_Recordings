...
def data_dets(eq_file_loc,sep): #rec_start = V['DateTime'].iloc[0]
    ...
    if len(V)==0:
        ...
        a = V.loc[:,['SENSOR ID', 'SUBJECT ID', 'HR(BPM)', #'SUBJECT AGE',
...
def test_plot_signals_interval(V,t1,t2): # V is a qiosk file read into pandas
    ...
    if len(V)>2: # not a hidden file
        ...
        if len(cols) ==1:
            c =cols[0]
            ax = X.loc[:,c].plot(figsize=[15,2])
            ax.set(ylabel=c,xlim=(t1,t2))
            ax.grid()
            ax.set_title(' '.join(['File signals from',t1.strftime('%Y-%m-%d %H:%M:%S'),'to',t2.strftime('%Y-%m-%d %H:%M:%S')]))
        else:
            fig, (axes) = plt.subplots(len(cols),1,figsize=[15,2*len(cols)],sharex=True)
            for i in range(len(cols)):
                ...
                ax.set(ylabel=c,xlim=(t1,t2))
                ax.grid()
            axes[0].set_title(' '.join(['File signals from',t1.strftime('%Y-%m-%d %H:%M:%S'),'to',t2.strftime('%Y-%m-%d %H:%M:%S')]))
        ax.set(xlabel='Datetime (UTC)')
        plt.show()
    else:
        print('No data') 
...
def file_plot_signals_interval(fileloc,t1,t2,sep='/'): # V is a qiosk file read into pandas
    V = pd.read_csv(fileloc)
    filename =fileloc.split(sep)[-1]
    # its on you to be sure these time stamps are within the recording interval of the file
    if len(V)>2: # not a hidden file
        if isinstance(V.index, pd.DatetimeIndex):
            V['DateTime'] = V.index
        ...
        if len(cols) ==1:
            ...
            ax.set(ylabel=c,xlim=(t1,t2))
            ax.grid()
            ax.set_title(' '.join([filename,'from',t1.strftime('%Y-%m-%d %H:%M:%S %Z'),'to',t2.strftime('%Y-%m-%d %H:%M:%S %Z')]))
        else:
            fig, (axes) = plt.subplots(len(cols),1,figsize=[15,2*len(cols)],sharex=True)
            for i in range(len(cols)):
                ax = axes[i]
                ...
                ax.set(ylabel=c,xlim=(t1,t2))
                ax.grid()
            axes[0].set_title(' '.join([filename,'from',t1.strftime('%Y-%m-%d %H:%M:%S %Z'),'to',t2.strftime('%Y-%m-%d %H:%M:%S %Z')]))
        ax.set(xlabel='Datetime ' + X.index[0].strftime('%Z'))
        plt.show()
    else:
        print('No data') 
...
def qiosk_recordings(projectpath,projecttag='new',sep = '/'):
    file_locs = []
...
def qiosk_rec_check(projectpath,projecttag='new',sep = '/'):
    file_locs = []
    for root, dirs, files in os.walk(projectpath):
        for file in files:
            if(file.lower().endswith(".csv")):
